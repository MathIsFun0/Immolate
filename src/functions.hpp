...
inline JokerData Instance::nextJoker(std::string source, int ante,
  ...
  if (rarity == Item::Legendary) {
    ...
    if (params.version > 10099) {
      joker =
          randchoice(RandomType::Joker_Rare + source + anteStr, RARE_JOKERS);
    } else {
      joker = randchoice(RandomType::Joker_Rare + source + anteStr,
                         RARE_JOKERS_100);
    }
...
inline Card Instance::nextStandardCard(int ante) {
  ...
  if (random(RandomType::Standard_Has_Enhancement + anteStr) <= 0.6) {
    ...
    enhancement = randchoice(RandomType::Enhancement +
                                 ItemSource::Standard_Pack + anteStr,
                             ENHANCEMENTS);
  }

  // Base
  Item base =
      randchoice(RandomType::Card + ItemSource::Standard_Pack + anteStr, CARDS);
...
inline void Instance::setStake(Item stake) { params.stake = stake; }
